<div class="text-3xl font-bold text-gray-800 sm:text-4xl">
    <h1>
                    ...
                    <form hx-post="/shareOnSocialMedia" hx-trigger="submit" hx-target="#parent-div" hx-swap="innerHTML">
                        <div x-data="{ choice: 'decide' }">
                            <div x-show="choice === 'decide'">
                                <div class="mt-4 grid">
                                    <button type="button" @click="choice = 'select'" class="inline-flex justify-center items-center gap-x-3 text-center bg-blue-600 hover:bg-blue-700 border border-transparent text-sm lg:text-base text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4">Yes, share on my social media.</button>
                                    <input type="hidden" name="title" value="<?= $title ?>">
                                </div>

                                <div class="mt-4 grid">
                                    <button type="submit" class="inline-flex justify-center items-center gap-x-3 text-center bg-blue-600 hover:bg-blue-700 border border-transparent text-sm lg:text-base text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4">No, thanks!</button>
                                </div>
                            </div>

                            <div x-show="choice === 'select'">
                                <div class="mt-2 space-y-2">
                                    <label for="message" class="block text-sm text-gray-700 font-medium">To Post a Message</label>

                                    <p class="block text-sm text-gray-700 font-medium">
                                        <?= $message = "Hey, I'm speaking on '{$title}'! To know more about it, visit " ?> <a href="http://localhost:8888/list" class="font-medium text-blue-600">conference.com</a>
                                    </p>
                                </div>

                                <div class="mt-2 space-y-2">
                                    <label for="message" class="block text-sm text-gray-700 font-medium">Select Your Option Below</label>
                                </div>

                                <div class="mt-6 grid">
                                    <button type="click" class="inline-flex justify-center items-center gap-x-3 text-center bg-blue-600 hover:bg-blue-700 border border-transparent text-sm lg:text-base text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4">
