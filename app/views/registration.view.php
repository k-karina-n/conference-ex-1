        ...
        <div class="overflow-hidden shadow sm:rounded-md">
            <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                <div class="grid gap-4 lg:gap-6">
                    ...
                    <form hx-post="/registration" hx-trigger="submit" hx-target="#parent-div" hx-swap="innerHTML" enctype="multipart/form-data">
                        <div x-data="{ 
                                form: { 
                                    firstName: $persist(''),
                                    lastName: $persist(''),
                            ...
                            <div x-data="{ currentStep:  $persist('first') }">
                                <div x-show="currentStep === 'first'" x-transition:enter="transition duration-200 transform ease-out" x-transition:enter-start="scale-75" x-transition:leave="transition duration-100 transform ease-in" x-transition:leave-end="opacity-0 scale-90">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                                        <div class="mt-2 space-y-2">
                                            ...
                                            <input type="text" name="firstName" id="firstName" x-model="form.firstName" class="py-3 px-4 block w-full rounded-md border border-gray-200 
                                            rounded-md text-sm hover:border-blue-500 focus:border-blue-500 focus:ring-blue-500">
                                        </div>

                                        <div class="mt-2 space-y-2">
                                            <label for="lastName" class="block text-sm text-gray-700 font-medium">Last Name</label>
                                            <input type="text" name="lastName" id="lastName" x-model="form.lastName" class="py-3 px-4 block w-full rounded-md border border-gray-200 
                                            rounded-md text-sm hover:border-blue-500 focus:border-blue-500 focus:ring-blue-500">
                                        </div>

                                        <div class="mt-2 space-y-2">
                                            <label for="phone" class="block text-sm text-gray-700 font-medium">Phone number</label>
                                            <input type="tel" name="phone" id="phone" x-model="form.phone" x-data x-mask="+99 (999) 999-9999" placeholder="+NN (NNN) NNN-NNNN" class="py-3 px-4 
                                            block w-full rounded-md border border-gray-200 rounded-md text-sm hover:border-blue-500 
                                            focus:border-blue-500 focus:ring-blue-500">
                                        </div>

                                        <div class="mt-2 space-y-2">
                                            <label for="email" class="block text-sm text-gray-700 font-medium">Email</label>
                                            <input type="email" name="email" id="email" x-model="form.email" placeholder="example@email.com" class="peer py-3 
                                            px-4 block w-full rounded-md border border-gray-200 bg-white py-2 px-3 shadow-sm text-sm hover:border-blue-500 
                                    ...
                                    <div class="mt-2 space-y-2">
                                        ...
                                        <select id="country" name="country" autocomplete="country-name" x-model="form.country" class="py-3 px-4 
                                        block w-full rounded-md border border-gray-200 bg-white py-2 px-3 shadow-sm text-sm hover:border-blue-500 
                                        focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <div class="mt-6 grid">
                                        <button type="button" @click="currentStep = 'second'" class="inline-flex justify-center items-center 
                                        gap-x-3 text-center bg-blue-600 hover:bg-blue-700 border border-transparent text-sm lg:text-base 
                                        text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 
                                        focus:ring-offset-white transition py-3 px-4">
                                            <div>
                                                <span>Next step</span>
                                            </div>
                                        </button>
                                ...
                                <div x-show="currentStep === 'second'" x-transition:enter="transition duration-200 transform ease-out" x-transition:enter-start="scale-75" x-transition:leave="transition duration-100 transform ease-in" x-transition:leave-end="opacity-0 scale-90">
                                    <div class="mt-2 space-y-2 cursor-pointer">
                                    ...
                                    <div class="mt-2 space-y-2">
                                        ...
                                        <div class="mt-2 space-y-2">
                                            <input type="date" name="date" id="date" x-model="form.date" class="py-3 px-4 block w-full rounded-md border border-gray-200 rounded-md text-sm hover:border-blue-500 focus:border-blue-500 focus:ring-blue-500" required>
                                        </div>
                                    </div>
